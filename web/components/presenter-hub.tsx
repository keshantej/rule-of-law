  ...
  return (
    <main className="px-5 py-8 md:px-8 md:py-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-5 max-w-3xl text-lg leading-8 text-paper/72">
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <section className="rounded-[2rem] border border-paper/10 bg-[#111b2b] p-6 md:p-8">
            ...
            <div className="mt-4 grid gap-6 md:grid-cols-[minmax(0,1fr)_18rem]">
              <div>
                <h2 className="text-3xl font-semibold tracking-[-0.04em] text-paper">Pick the format that fits the room, then rehearse from presenter mode.</h2>
                <p className="mt-4 max-w-2xl text-sm leading-8 text-paper/72">
                  Most speakers should start with the 45-minute version, then shorten or expand if the schedule requires it.
                </p>
                <div className="mt-6 flex flex-wrap gap-3">
                  <Link href={`/presentation/speaker?duration=${recommendedTrack?.minutes ?? 45}`} className="inline-flex items-center gap-2 rounded-full bg-paper px-5 py-3 text-sm font-semibold text-ink transition hover:bg-gold">
                    Open recommended presenter mode
                    <ArrowRight className="h-4 w-4" />
                  </Link>
                  <Link href="/certification" className="inline-flex items-center gap-2 rounded-full border border-paper/10 bg-paper/10 px-5 py-3 text-sm font-semibold text-paper transition hover:border-gold/45 hover:text-gold">
                    Review certification
                  </Link>
                </div>
              </div>
              <div className="rounded-[1.5rem] border border-paper/10 bg-paper/5 p-5">
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-gold">Suggested sequence</p>
                <div className="mt-4 space-y-3 text-sm leading-7 text-paper/74">
                  <div className="flex items-start gap-3">
                    <Mic2 className="mt-1 h-4 w-4 text-gold" />
                    <p><span className="font-semibold text-paper">Choose a timed format</span> that matches the room and schedule.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <NotebookText className="mt-1 h-4 w-4 text-gold" />
                    <p><span className="font-semibold text-paper">Review the script and notes</span> before opening speaker mode.</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <Presentation className="mt-1 h-4 w-4 text-gold" />
                    <p><span className="font-semibold text-paper">Switch to Presentation</span> or Standalone mode when the audience is in the room.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <aside className="rounded-[2rem] border border-paper/10 bg-paper/5 p-6">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-4">
          {tracks.map((track) => (
            <div key={track.minutes} className="rounded-[1.5rem] border border-paper/10 bg-paper/5 p-5">
              <p className="text-xs font-semibold uppercase tracking-[0.22em] text-gold">{track.minutes} minute format</p>
              <h2 className="mt-3 text-xl font-semibold tracking-[-0.03em] text-paper">{track.title}</h2>
              ...
              <div className="mt-4 rounded-2xl border border-paper/10 bg-[#111b2b] px-4 py-3 text-sm text-paper/70">
                <p className="font-semibold text-paper">{track.steps.length} timed blocks</p>
                <p className="mt-1">{track.transition_note}</p>
              </div>
              <Link href={`/presentation/speaker?duration=${track.minutes}`} className="mt-5 inline-flex rounded-full border border-paper/10 bg-paper/10 px-4 py-2 text-sm font-semibold text-paper transition hover:border-gold/45 hover:text-gold">
                Open in presenter mode
              </Link>
            </div>
          ))}
          <div className="rounded-[1.5rem] border border-paper/10 bg-[#111b2b] p-5 text-paper">
            ...
            <Link href="/presentation" className="mt-5 inline-flex rounded-full border border-paper/10 bg-paper/5 px-4 py-2 text-sm font-semibold text-paper transition hover:border-gold/40 hover:text-gold">
              Open presentation
            </Link>
          </div>
        </div>
            ...
            <section>
              ...
              <div className="space-y-5">
                {items.filter((item) => item.type === "talk_track").map((item) => (
                  <article key={item.toolkit_id} className="rounded-[1.75rem] border border-paper/10 bg-[#111b2b] p-6">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-gold">{item.type.replaceAll("_", " ")}</p>
                        <h3 className="mt-3 text-2xl font-semibold tracking-[-0.04em] text-paper">{item.title}</h3>
                      </div>
                    ...
                    <p className="mt-4 whitespace-pre-line text-sm leading-7 text-paper/74">{item.body}</p>
                  </article>
                ))}
              </div>
            </section>
              ...
              <div className="space-y-5">
                {supportItems.map((item) => (
                  <article
                    ...
                    key={item.toolkit_id}
                    className="rounded-[1.75rem] border border-paper/10 bg-paper/[0.04] p-6"
                  >
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-gold">{item.type.replaceAll("_", " ")}</p>
          ...
          <aside className="space-y-5 lg:sticky lg:top-24 lg:h-fit">
            <div className="rounded-[1.75rem] border border-paper/10 bg-paper/5 p-6">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Link href="/presentation/speaker" className="rounded-full border border-paper/10 bg-paper/10 px-4 py-2 text-sm text-paper transition hover:border-gold/45 hover:text-gold">
                  Presenter mode
                </Link>
                <a href="#preparation" className="rounded-full border border-paper/10 bg-paper/10 px-4 py-2 text-sm text-paper transition hover:border-gold/45 hover:text-gold">
                  Prep checklist
                </a>
                <a href="#questions" className="rounded-full border border-paper/10 bg-paper/10 px-4 py-2 text-sm text-paper transition hover:border-gold/45 hover:text-gold">
                  Difficult questions
                </a>
                <Link href="/resources" className="rounded-full border border-paper/10 bg-paper/10 px-4 py-2 text-sm text-paper transition hover:border-gold/45 hover:text-gold">
                  Resources
                </Link>
              </div>
            </div>
            <div className="rounded-[1.75rem] border border-paper/10 bg-paper/[0.04] p-6">
            ...
            <div className="rounded-[1.75rem] border border-paper/10 bg-[#111b2b] p-6 text-paper">
              ...
              <div className="mt-5 space-y-3">
                {downloads.map((asset) => (
                  <Link key={asset.asset_id} href={asset.href} className="block rounded-2xl border border-paper/10 bg-paper/5 px-4 py-4 transition hover:border-gold/40 hover:text-gold">

...
export function PresentationShell({ scenes, track, mode, resources, downloads, currentDuration, availableDurations }: PresentationShellProps) {
  ...
  if (isStandalone) {
    return (
      <div className="relative min-h-screen overflow-hidden bg-[#0c1421] text-paper">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(205,161,93,0.18),transparent_24%),radial-gradient(circle_at_82%_20%,rgba(155,35,50,0.14),transparent_20%),linear-gradient(180deg,rgba(23,40,71,0.5),rgba(12,20,33,0.96))]" />
        <div className="absolute right-[5%] top-[8%] text-[8rem] font-semibold tracking-[-0.08em] text-paper/[0.05] md:text-[14rem]">
        ...
        <div className="relative flex min-h-screen flex-col justify-between px-6 py-8 md:px-10 md:py-10">
          <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4">
            <div className="flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-paper/55">
              <span>{deferredScene.kicker}</span>
              <span>Scene {currentIndex + 1} of {scenes.length}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="hidden rounded-full border border-paper/10 bg-paper/5 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-paper/58 md:block">
                Standalone presentation
              </div>
              <Link href={`/presentation?duration=${currentDuration}`} className="rounded-full border border-paper/10 bg-paper/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-paper/68 transition hover:border-gold/40 hover:text-gold">
                Exit standalone
              </Link>
            </div>
          </div>
              ...
              {deferredScene.display_lines.map((line, index) => (
                <p
                  ...
                  className={cn(
                    ...
                    index === 0 ? "text-5xl font-semibold md:text-8xl" : "text-2xl font-medium text-paper/72 md:text-4xl"
                  )}
                >
                  {line}
                </p>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-paper/10 bg-paper/5 px-4 py-2 text-sm font-semibold text-paper transition hover:border-gold/40 hover:text-gold disabled:cursor-not-allowed disabled:opacity-45"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-paper px-4 py-2 text-sm font-semibold text-ink transition hover:bg-gold disabled:cursor-not-allowed disabled:opacity-45"
                >
                  Next
                  <ChevronRight className="h-4 w-4" />
                </button>
              </div>
              <div className="text-sm text-paper/58">Use arrow keys or spacebar to move through the presentation.</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="relative mx-auto flex h-full max-w-6xl flex-col justify-between gap-8">
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-4xl">
              <div className="mb-4 flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-paper/58">
                <span className="rounded-full border border-paper/10 bg-paper/5 px-3 py-2 text-gold">{deferredScene.kicker}</span>
          ...
          >
            ...
            {isSpeaker ? (
              <div className="rounded-[1.75rem] border border-paper/10 bg-paper/6 p-6">
                {currentTrackStep ? (
                  <div className="rounded-[1.25rem] border border-paper/10 bg-paper/5 p-4">
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-gold">Talk track cue</p>
                    <p className="mt-2 text-sm leading-7 text-paper/76">{currentTrackStep.cue}</p>
                  </div>
                ) : null}
                <div className={cn(currentTrackStep ? "mt-8 border-t border-paper/10 pt-6" : "")}>
                  <p className="mb-3 text-xs font-semibold uppercase tracking-[0.28em] text-gold">Speaker notes</p>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-t border-paper/10 pt-6">
            <div className="flex items-center gap-2 text-sm text-paper/75">
              <button
                type="button"
                onClick={() => startTransition(() => setSceneIndex((value) => Math.max(value - 1, 0)))}
                disabled={currentIndex === 0}
                className="inline-flex items-center gap-2 rounded-full border border-paper/10 bg-paper/5 px-4 py-2 transition hover:border-gold/40 hover:text-gold disabled:cursor-not-allowed disabled:opacity-45"
              >
                <ChevronLeft className="h-4 w-4" />
                Previous
              </button>
            ...
            <div className="flex flex-wrap items-center gap-3 text-sm text-paper/65">
              <span>{isSpeaker ? "Use arrow keys or the scene list to move through the talk." : `Scene ${currentIndex + 1} of ${scenes.length}`}</span>
              {!isSpeaker ? (
                <Link href={`/presentation/standalone?duration=${currentDuration}`} className="inline-flex items-center gap-2 rounded-full border border-paper/10 bg-paper/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-paper/70 transition hover:border-gold/40 hover:text-gold">
